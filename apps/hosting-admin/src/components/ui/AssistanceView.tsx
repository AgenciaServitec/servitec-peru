...
export const AssistanceView: React.FC<AssistanceViewProps> = ({
  ...
  const [status, setStatus] = useState({ message: 'Esperando acción...', type: 'info' });
  ...
  const { stream, startVideo, stopVideo } = useVideoStream(videoRef, (msg, type) =>
  ...
  const showToast = (message: string, type: string = 'info', duration = 3000) => {
  ...
  const captureFace = async (): Promise<Float32Array | null> => {
    ...
    try {
      ...
      if (!detection) {
        setStatus({ message: 'No se detectó rostro, intenta de nuevo.', type: 'error' });
        return null;
      }
  ...
  const handleSaveAssistance = async (type: 'entry' | 'outlet') => {
    ...
    if (!currentDescriptor) return showToast('No se detectó rostro, intenta de nuevo.', 'error');

    // if (!user) return showToast("No hay usuario cargado.", "error");

    // const distance = calculateEuclideanDistance(currentDescriptor, new Float32Array(user.faceDescriptor));
  ...
  return (
    <Container>
      <Col span={24}>
        <Button type="primary" size="large" onClick={() => goTo('/assistances')}>
      ...
      {/* 🔹 Por ahora dejamos la parte de búsqueda por DNI y usuario comentada */}

      {/* {!exists ? (
        <div className="form-wrapper">
          <Form onSubmit={(e) => (e.preventDefault(), searchUser())} style={{ width: "100%" }}>
            <Row gutter={[16, 16]}>
              <Col span={24}>
      ...
      <>
        ...
        <div className="content">
          <div className="left-panel">
            ...
            {feedback.show && (
              <div className={`alert ${feedback.type}`}>
                ✅ Asistencia marcada: {feedback.type === 'entry' ? 'Entrada' : 'Salida'}
              </div>
            )}
          ...
          <div className="right-panel">
            <UserLocationMap location={location} onValidateGeofence={setIsGeofenceValid} />
          </div>
        </div>
      </>
      {/* )} */}
