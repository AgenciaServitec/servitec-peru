...
type StorageReference = firebase.storage.Reference;
...
export const isRcFile = (data: unknown) => isObject(data) && 'uid' in data;
...
export const uploadFile = async ({
  ...
  await new Promise((resolve, reject) => {
    ...
    storage
      ...
      .on(
        'state_changed',
        ({ bytesTransferred, totalBytes }) => onProgress((bytesTransferred / totalBytes) * 95),
        (error) => {
          onError(error);
          reject(error);
        },
    ...
    const uploadComplete = async (newFile: UploadedFile, fileConfig: FileConfig) => {
      try {
...
const mapUploadFile = (file: RcFile) => ({
  uid: file.uid,
  name: file.name,
});
...
export const deleteFileAndFileThumbFromStorage = async (
  ...
  const pathThumbImages = imageResizes.map(
    (resizeImage) =>
      `${filePath}/thumbs/${fileName.replace(`.${extension}`, '')}_${resizeImage}.${extension}`
  );
...
export const keepTryingGetThumbURL = async (
  ...
  try {
    ...
    // Re-lanzar el error si no es el caso que esperamos
    throw error;
  }
};
